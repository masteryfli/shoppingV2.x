            ...
            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap">
                  <span class="error error-show" v-show="errorTip">用户名和密码不匹配</span>
                </div>
                <ul>
...
<script>
    ...
    export default{
        ...
        methods:{
           checkLogin() {
            axios.get('user/checkLogin').then((result) => {
              let res = result.data;
              if (res.code == 0) {
          ...
          login(){
            ...
            axios.post('/user/login', param).then((res)=>{
              ...
              if (result.code == 0) {
                ...
                this.nickName = result.data.userName;
                this.getCartCount();
              } else {
                this.errorTip = true
          ...
          //获取购物车数量
          getCartCount() {
            axios.get('/user/getCartCount').then((res) => {
              let result = res.data
              if (result.code == 0) {
                let count = result.data
                this.$store.commit("initCart", count)
              }
            })
          }
        }
