        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item, index) in addressListFilter" :key="index" v-bind:class="{'check': checkIndex==index}" @click="checkIndex=index;selectedId=item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" v-bind:to="{path:'confirm', query:{'addressId':selectedId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <modal :mdShow="isMdShow" v-on:close="closeModal">
          <p slot="message">确认删除该地址？</p>
          <div slot="btnGroup">
              <a class="btn btn--m" href="javascript:;" @click="delConfirm">确定</a>
...
<script>
  ...
  export default{
    ...
    methods:{       
        //查询地址
        init() {
            axios.get('/user/addressList').then((result) =>{
                let res = result.data
        ...
        setDefault(addressId) {
            axios.post('/user/setDefault', {'addressId': addressId}).then((result) => {
                let res = result.data
                if (res.code == 0) {
                    this.addressList = res.data
                }
        ...
        //删除地址确认
        delConfirm() {
            axios.post("/user/delAddress", {'addressId': this.addressId}).then((result) =>{
                let res = result.data
                if (res.code == 0) {
                    this.isMdShow = false
                    this.init();
