<template>
  <div>
              ...
              <li v-for="item in cartList" :key="item.productId">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delCartItem(item)">
    ...
    <Modal :mdShow="modalConfirm" v-on:close="closeModal">
        <p slot="message">确认删除该数据？</p>
        <div slot="btnGroup"  @close="closeModal">
...
<script>
  ...
  export default{
      ...
      components:{
       ...
       methods:{
           init() {
               axios.get("/user/cartList").then((result) =>{
                   let res = result.data;
                   if (res.code == 0) {
           ...
           //删除购物车数据
           delCart() {
               axios.post("/user/delCart", {productId:this.item.productId}).then((result)=>{
                   let res = result.data
                   if (res.code == 0) {
                       this.modalConfirm = false;
                       this.cartList.forEach((val, index, array)=>{
                           if (array[index].productId == this.item.productId) {
                               this.cartList.splice(index, 1)
                           }
                       })
                      this.$store.commit('updateCart', -this.item.productNum)
                   }
               })
           },
           //购物车删除按钮点击
           ...
           editCart(action, item) {
            ...
            //全选
           checkAll() {
               var checkFlag = !this.checkAllFlag;
               this.cartList.forEach((item)=>{
                   item.checked = checkFlag ? 1 : 0
               })
               axios.post('/user/editCheckAll', {
                   checkedAll: checkFlag
               }).then((result) => {
                   let res = result.data
                   if (res.code == 0) {
